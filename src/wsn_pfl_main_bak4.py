    ...
    if fake:
        ...
        for name in file_list:
            if tag in name and 'fake' not in name:  # fake result can not be used for fake training
                fake_path = os.path.join('../logs', name)
                is_find = True
                break
        if is_find:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for c, user_groups in enumerate(user_groups_list):
            ...
            for idx in idxs_users:
                ...
                #     approxmimate_grad = (cluster_wegihts_list[c][name] - w[name]) / lr / K
                #     init_grad_vector.extend(approxmimate_grad.cpu().numpy().reshape(-1).tolist())
