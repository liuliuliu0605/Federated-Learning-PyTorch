...
class Network:
    ...
    def get_ps_matrix(self, target_overlay_graph = None):
    ...
    def pfl_step(self, times=10, start_time=0, stop_time=None, protocol='tcp', port=9, verbose=False):
        ...
        app = DecentralizedConsensus(self.model_size, ps_node_container, self.get_ps_matrix(),
                                     protocol=protocol, port=port, verbose=verbose)
        app.init_app(start_time, stop_time)
        base_time = ns.core.Simulator.Now().GetSeconds() + start_time
        ns.core.Simulator.Run()
        cur_time = app.get_current_time()
    ...
    def ring_based_all_reduced(self, start_time=0, stop_time=None, protocol='tcp', port=9, verbose=False):
        ...
        # ring_list = [i for i in range(self.ps_num)]
        ring_list = [0]
        ...
        app = RingAllReduce(self.model_size, ps_node_container, ring_list, protocol=protocol, port=port, verbose=verbose)
        app.init_app(start_time, stop_time)
        base_time = ns.core.Simulator.Now().GetSeconds() + start_time
        print(base_time)
        ns.core.Simulator.Run()
        return app.get_current_time() - base_time
