from src.simulator.internet.utils import *
from src.simulator.internet.ring_allreduce import RingAllReduce
from src.simulator.internet.decentralized_consensus import DecentralizedConsensus
...
class Network:
    ...
    def __init__(self, client_group_list, underlay_name='gaia',
        ...
        assert self.ps_num <= self._underlay_graph.number_of_nodes()
        self._target_connectivity_graph = get_target_connectivity_graph(self._underlay_graph,
    ...
    def prepare(self, topo_name='ring', params={}, verbose=False):
        ...
        if len(params) > 0:
            method = "%s-%s" % (topo_name, params_str)
        else:
            method = topo_name

        # with open("./ns3_rs/%s.json" % method, 'w') as f:
        #     json.dump(self.get_ps_matrix(weight='mixWeight').tolist(), f)
    ...
    def _topo_formation(self, topo_name='ring', params={}):
        if topo_name == 'ring':
            ...
            target_overlay_graph = get_regular_graph(self._target_connectivity_graph,
                                                     k=params.get('k', 2))
        elif topo_name == 'neighbour':
            target_overlay_graph = get_neighbour_overlay(self._target_connectivity_graph,
                                                         hops=params.get('hops', 1))
        elif topo_name == 'star':
            target_overlay_graph = get_star_overlay(self._target_connectivity_graph)
        elif topo_name == 'random':
            target_overlay_graph = get_random_overlay(self._target_connectivity_graph,
    ...
    def _construct_network(self, verbose=False):
        ...
        assert len(self._ps_loc_list) <= 253

        for i, sn in enumerate(self._ps_loc_list):
    ...
    def pfl_step(self, times=10, start_time=0, stop_time=None, protocol='tcp', port=9, verbose=False,
        ...
        time_consuming = app.run(start_time, stop_time, times)
        if save_path is not None:
            with open(save_path, 'w') as f:
                f.write(json.dumps(app.get_comm_matrix_list(), indent=4))
        return time_consuming

    def hier_favg(self, start_time=0, stop_time=None, protocol='tcp', port=9, verbose=False,
                  offline_params={}, max_block_duration=None, save_path=None):
        ps_node_container = ns.network.NodeContainer()
        for i in range(self.ps_num):
            ps_node_container.Add(self._ps_clients_nodes_list[i].Get(0))
        ...
        return sum(time_consuming_list)
    ...
    def plot_network(self, name='connectivity', node_label=False, figsize=(10,10)):
        ...
        if name == 'underlay':
            ...
            nx.draw_networkx_nodes(self._target_connectivity_graph, node_color='red', pos=pos, ax=ax)
...
if __name__ == "__main__":
    ...
    client_group_list = [[1, 4, 9, 10, 13, 17, 19, 20, 52, 54, 60, 61, 64, 66, 68, 71, 76, 82, 89, 98],
                         ...
                         # [1, 4, 9, 10, 13, 17, 19, 20, 52, 54, 60, 61, 64, 66, 68, 71, 76, 82, 89, 98],
                         # [6, 7, 31, 33, 34, 35, 36, 39, 40, 43, 44, 55, 59, 62, 69, 70, 78, 92, 93, 99],
                         # [11, 14, 21, 25, 26, 30, 46, 49, 53, 58, 65, 72, 74, 79, 81, 85, 87, 90, 91, 96],
                         # [0, 8, 16, 22, 27, 29, 32, 41, 45, 47, 48, 57, 67, 73, 77, 83, 84, 86, 88, 97],
                         # [2, 3, 5, 12, 15, 18, 23, 24, 28, 37, 38, 42, 50, 51, 56, 63, 75, 80, 94, 95],
    ...
    network = Network(client_group_list=client_group_list, underlay_name='telstra',
                      parent_dir='.', model_size=1e7, link_capacity=1e9, verbose=False)
    # network = Network(client_group_list=client_group_list, underlay_name='ebone', parent_dir='.',
    #                   link_capacity=1e3, node_capacity=1e3,
    #                   wan_latency='auto', lan_latency=5e-17,
    #                   model_size=1e5, verbose=False)
    network.prepare(topo_name='neighbour', verbose=False)
    ...
    # if False:
    #     current_time = network.ring_based_all_reduced(start_time=0, stop_time=100000, protocol='tcp', verbose=True)
    #     print("ring allreduce:%f ms" % (current_time * 1000))
    # if True:
    #     current_time = network.pfl_step(start_time=0, stop_time=100000, protocol='tcp', port=19, verbose=True)
    #     print("pfl:%f ms" % (current_time * 1000))
