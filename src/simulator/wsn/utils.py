...
def plot_traces(traces, save_dir=None):
    ...
    for scenario, tracer in traces.items():
        ...
        for trace_name, trace in tracer.items():
            ...
            # plt.plot(X, trace[2], color_n_line)
            plt.xlabel(trace[1])
            plt.ylabel(trace[0])
    ...
    if save_dir is not None:
        plt.savefig(os.path.join(save_dir, 'traces.pdf'))
...
def plot_clusters(network, save_dir=None):
    ...
    for i in range(len(heads)):
        plt.annotate(text="%d" % i, xy=(X[i] * 1.01, Y[i] * 1.01),
                     xytext=(X[i] * 1.01, Y[i] * 1.01), weight='bold')

    # print topo
    for i in range(len(heads)):
        for j in range(i, len(heads)):
            if network.topo[i, j] > 0:
                plt.plot((X[i], X[j]), (Y[i], Y[j]), linestyle='-.', color='gray', alpha=0.5)
    ...
    plt.tight_layout()
    if save_dir is not None:
        plt.savefig(os.path.join(save_dir, 'cluster.pdf'))
    plt.show()


def plot_time_of_death(network, save_dir=None):
    """Plot time of death as a colormap."""
    x = np.array([node.pos_x for node in network[0:-1]])
    y = np.array([node.pos_y for node in network[0:-1]])
    z = np.array([node.time_of_death for node in network[0:-1]])
    print("death statistics: max(%d), min(%d), avg(%d), std(%.2f)"
    ...
    if save_dir is not None:
        plt.savefig(os.path.join(save_dir, 'death.pdf'))
