...
from utils import get_dataset_cluster, get_dataset,average_weights, mixing_weights, exp_details_cluster, get_device, init_seed,\
    complete, star, ring, independent, plot_label_dis, update_topo, optimal_mixing_cycles, lr_decay
...
if __name__ == '__main__':
    ...
    tag = 'Cluster[{}]_Sim[{:.2f}]_Topo[{}]_Mix[{}]_lr[{:.3f}]_frac[{:.1f}].pkl'.\
        format(args.num_clusters, args.cluster_similarity, args.topo, args.mix_ep, args.lr, args.frac)
    ...
    mixing_timer, dynamic_tau, num_mixing = 1, 1, 0
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if network.get_remaining_energy() <= 50000-budget:
        ...
        for c, user_groups in enumerate(user_groups_list):
            ...
            if not fake:
                ...
                for idx in idxs_users:
                    ...
                    # if args.mix_ep == 0:
                    if True:
                        params, gradient = local_model.estimate(model=copy.deepcopy(global_model), global_round=epoch,
        ...
        if True:
            ...
            tau = args.mix_ep
            K = len(train_dataset) / 100 / args.local_bs
            # L = max(1 / (8 * lr * K), L)
            # lr = 1 / (8 * L * K)
            # L = min(1 / (8 * lr * K), L)
        ...
        if args.mix_ep == 0 and mixing_timer <= 0:
            ...
            # if np.mean(agg_delta_list) / intra_energy_per_round < np.mean(mix_delta_list) / inter_energy_per_round:
            #     dynamic_tau -= 1
